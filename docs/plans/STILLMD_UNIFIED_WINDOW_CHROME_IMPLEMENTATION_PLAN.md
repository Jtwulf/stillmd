...
- [`MarkdownPreviewer/Views/PreviewView.swift`](../../MarkdownPreviewer/Views/PreviewView.swift) では、本文 `corePreview` に対して `safeAreaInset(edge: .top)` で `topChrome` を差し込んでいる
- そのため、検索バーやエラー帯は本文と別レイヤーの上部クロムとして見えやすい
- [`MarkdownPreviewer/Views/FindBar.swift`](../../MarkdownPreviewer/Views/FindBar.swift) の検索 UI は `RoundedRectangle + windowBackgroundColor + stroke` で独立したカードとして描かれている
- [`MarkdownPreviewer/Views/WindowAccessor.swift`](../../MarkdownPreviewer/Views/WindowAccessor.swift) は各ウインドウへ `representedURL` を設定している
- [`MarkdownPreviewer/Services/WindowManager.swift`](../../MarkdownPreviewer/Services/WindowManager.swift) の duplicate detection 後の `bringToFront` は `window.representedURL == url` に依存している
- [`MarkdownPreviewer/App/MarkdownPreviewerApp.swift`](../../MarkdownPreviewer/App/MarkdownPreviewerApp.swift) では、現時点で titlebar / full-size content view を統合する明示設定はない
...
- `MarkdownPreviewer/App/MarkdownPreviewerApp.swift`
- `MarkdownPreviewer/Views/RootView.swift`
- `MarkdownPreviewer/Views/PreviewView.swift`
- `MarkdownPreviewer/Views/FindBar.swift`
- `MarkdownPreviewer/Views/WindowAccessor.swift`
- `MarkdownPreviewer/Services/WindowManager.swift`

### 4.2 追加の可能性があるファイル

- `MarkdownPreviewer/Views/EmptyStateView.swift`
- `MarkdownPreviewer/Views/ErrorView.swift`
- 新規: `MarkdownPreviewer/Services/WindowChromeConfigurator.swift` または同等の helper
