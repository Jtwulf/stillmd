...
- Empty state は英語・最小表示・アイコンなしで整える
- Empty state の文言は `Title: Open Markdown` / `Button: Open` を採用する
- pending URL のポーリングはやめ、通知ベースで受け取る
...
- [x] pending URL の起動時ポーリングを通知ベースの受け渡しへ置き換える
- [x] `swift build`
- [x] `swift test`
...
- [x] Empty state を静かに洗練する
  - [x] アイコンを削除する
  - [x] 文言を `Open Markdown` / `Open` に揃える
  - [x] 常設の補助文言と `⌘O` 表示を置かない
